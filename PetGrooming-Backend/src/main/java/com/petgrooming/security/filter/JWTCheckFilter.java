...
//JWT 토큰을 검증하고 인증 정보를 설정하는 필터 클래스입니다.
@Log4j2
public class JWTCheckFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			MemberDTO memberDTO = new MemberDTO(m_num.longValue(), m_name, m_birth, m_gender, m_email, m_pw, m_phone,
					m_addr, dog_breed, dog_name, dog_birth, dog_notice, m_state, m_agree, roleNames);
	...
	protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
		...
		if (path.startsWith("/api/faq/list")) {
			return true;
		}
		
		if (path.startsWith("/api/designer/list")) {
			return true;
		}
